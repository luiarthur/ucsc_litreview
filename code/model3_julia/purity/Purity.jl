...
function fit(n₁::Vector{Int}, N₁::Vector{Int}, N₀::Vector{Int}, M::Vector{Float64}, 
             m_phi::Float64, s2_phi::Float64,
             a_sig::Float64, b_sig::Float64,
             a_mu::Float64, b_mu::Float64, cs_mu::Float64,
             a_m::Float64, b_m::Float64, cs_m::Float64,
             a_w::Float64, b_w::Float64,
             alpha::Float64, a_v::Float64, b_v::Float64, cs_v::Float64,
             B::Int, burn::Int, printFreq::Int)
  ...
  function update(curr::State)
    ...
    function p(mu::Float64, v::Vector{Float64}, m::Vector{Float64})
      return (mu * v .* m) ./ (2.0*(1.0-mu) + mu*ms)
    end
    ...
    const phi_new = begin
      ...
      const phi_sd = sqrt(sig2_new*s2_phi / phi_denom)
      return rand(MvNormal(phi_mean, Matrix(Diagonal(phi_sd))))
    end
    ...
    const mu_new = begin
      ...
      return DPMM.metLogit(curr.mu, llMu, lpMu, cs_mu)
    end

    # Update v
    const v_new = begin
      function lf(vs::Float64, s::Int)
        const pss = ps(mu_new, vs, curr.m[s])
      ...
      DPMM.neal8(alpha, curr.v, lg, lg0, rg0, metLogit, cs_v)
    end
  ...
  return DPMM.gibbs(init, update, B, burn, printFreq)
end
