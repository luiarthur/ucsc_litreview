...
% ARTHUR LUI
\newpage
\renewcommand{\iid}{\overset{iid}{\sim}}
...
\begin{align*}
  n_{1s} \mid N_{1s}, p_{s} &\sim \Binom(N_{1s}, p_{s}) \\
  \log\p{\frac{N_{1s}}{N_{0s}}} \mid \mu, \phi_s, M_s, \sigma^2 &\sim \N\p{\log\p{\frac{2(1-\mu)+\mu M_s}{2}}+\phi_s, \sigma^2}\\
\end{align*}
...
\subsubsection{Complete Conditional for $\sigma^2$}
\begin{align*}
p(\sigma^2 ~|~ ...) &\propto \prod_{s=1}^S \mathcal{L}_2(\mu,\phi_s,\sigma^2 ~|~ N_{1s}, N_{0s}) \times p(\sigma^2)\\
                    &\propto \p{\sigma^2}^{-S/2}\exp\bc{\ds-\frac{\sums\p{\log(N_{1s}/N_{0s})-\log z_s-\phi_2}^2}{2\sigma^2}} (\sigma^2)^{-a_\sigma-1} \exp(-b/\sigma^2)\\
                    &\propto (\sigma^2)^{-a_\sigma-S/2-1} \exp\bc{-\frac{1}{\sigma^2}\p{\frac{\sums\p{\log(N_{1s}/N_{0s})-\log z_s-\phi_2}^2}{2}}}\\
\end{align*}
where $z_s = \ds\frac{2(1-\mu)+\mu M_s}{2}$.

$$
\therefore ~~~ \sigma^2|... \sim \text{IG}\p{a_\sigma+S/2, \frac{\sums\p{\log(N_{1s}/N_{0s})-\log z_s-\phi_2}^2}{2}}
$$

\subsubsection{Complete Conditional for $\phi_s$}
Let $Y_s = N_{1s}/N{0s}$
\begin{align*}
...
\subsubsection{Complete Conditional for $\mu$}
\begin{align*}
...
\end{align*}
Note that here a metropolis step is needed.

\subsubsection{Complete Conditional for $v_s$}
\begin{align*}
...
\end{itemize}
Note that we can sample from the complete conditional for $v_s$ efficiently using one of the algorithms (e.g. Algorithm 8) presented by Radford Neal (2000).
