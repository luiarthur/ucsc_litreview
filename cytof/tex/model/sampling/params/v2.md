...
3. If $z_{jl}$ is unchanged, then the proposed state for $\mus_{jl}$ is simply
   ...
   $\tmus_{jl} \sim \N(\cdot \mid \psi_j, \epsilon^2, \tilde z_{jl})$ 
4. Compute the proposal density as 
   ...
   \prod_{j=1}^J \prod_{l=k}^{K}
   p(\tilde\mus_{jl} \mid \psi_j, \epsilon^2, \tilde z_{jl})^{\Ind{\tilde z_{jl} \ne z_{jl}}}
   \end{split}
   $$
5. The proposed state $(\tilde\phi_k, \bm\tmus_{k:K})$ will be
   ...
   \begin{align*}
     ...
     }^{\Ind{\lin\ge k, ~~z_{j,\lin} \ne \tilde z_{j,\lin}}} \times 
     \\
     & ~~~~ \prod_{j=1}^J\prod_{l=k}^K
     \bc{
       \frac{
         p(\tmus_{jl} \mid \psi_j, \tau_j^2, \tilde z_{jl})
         ~
         p(\mus_{jl} \mid \psi_j, \epsilon^2, z_{jl})
       }{
         p(\mus_{jl} \mid \psi_j, \tau_j^2, z_{jl})
         ~
         p(\tmus_{jl} \mid \psi_j, \epsilon^2, \tilde z_{jl})
       }
     }^{\Ind{z_{jl} \ne \tilde z_{jl}}}
   \end{align*}
