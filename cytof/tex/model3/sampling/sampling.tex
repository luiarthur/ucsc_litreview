...
%
A comparison of freely available clustering methods for high-dimensional
cytometry data was made by \cite{weber2016comparison}. These methods are based
on hierarchical clustering methods , k-means, density-based clustering methods,
model-based clustering methods, nearest-neighbor methods, and self-organizing
maps. Each method exhibits their own strengths. For example,
FlowSOM \citep{van2015flowsom} uses a self-organizing map to cluster flow
cytometry data. Self-organizing maps are trained using unsupervised neural
...
graph called a map. Among existing methods it is also considered the fastest
and most flexible at finding subpopulations in cytometry data. FlowSOM is also
able to learn the number of clusters in the data. However, does not measure
...
number of clusters are made, and the final output is a dendrogram, which
can then be cut at different levels to produce clusters. But again, they
