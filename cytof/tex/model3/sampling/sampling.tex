...
A major challenge in deciphering mass cytometry data is developing efficient
...
%
To this end, a comparison of freely available clustering methods for
high-dimensional cytometry data was made by \cite{weber2016comparison}. These
methods are based on hierarchical clustering methods , k-means, density-based
clustering methods, model-based clustering methods, nearest-neighbor methods,
and self-organizing maps. Each method exhibits their own strengths. For
example,
FlowSOM \citep{van2015flowsom} uses a self-organizing map to cluster flow
cytometry data. Self-organizing maps are trained by unsupervised neural
networks to obtain a low-dimensional representation of the input space of the
data. Since neural networks are capable of learning non-linear surfaces, it may
be considered a nonlinear generalization of Principal components analysis
\citep{yin2008learning}.  In the FlowSOM implementation, the input space is
projected into a two-dimensional space and can be conveniently visualized in a
graph called a map. Among existing methods it is also considered the fastest and
most flexible at finding subpopulations in cytometry data. However, it is not
able to measure uncertainty about the learned clusters.
%
These are common traits among the clustering methods used in analyzing
cytometry data. K-means, while fast, memory-efficient, and intuitive, produces
only point estimates of clusters. In addition, the number of clusters needs to
be predetermined. Hierarchical clustering implementations typically suffer from
memory-related bottlenecks for large datasets. Memory-efficient implementations
suited for large datasets have been implemented in the statistical programming
language R by \citep{linderman2013package}, making it a viable candidate method
for cytometry data. In hierarchical clustering, no assumptions about the 
number of clusters are made, and the final output is a dendrogram 
%
...
phenotypes generated by the \(J\) markers. It is computationally infeasible even when \(J\) is moderately large. Taking a Bayesian approach, we consider a prior probability model over binary matrices %with infinite number of columns 
and learn the predominant ones in posterior that  generate observed data. %For computational efficiency, we require a flexible prior which will also learn the number of cell-types \((K)\). 
