...
own strengths.
% FLOWSOM
For example, FlowSOM \citep{van2015flowsom} uses a
self-organizing map to reduce the dimensionality of and cluster flow cytometry
data. Self-organizing maps are trained using unsupervised neural networks to
obtain a low-dimensional representation of the input space of the data. Since
neural networks are capable of learning non-linear functions, it may be
considered a nonlinear generalization of principal components analysis
...
graph called a map. While self-organizing graphs depend on users to specify the
number of clusters, FlowSOM can automatically learn the number of clusters in
the data.  Among existing methods FlowSOM is considered the fastest and most
flexible at finding subpopulations in cytometry data. However, it does not
provide any measures of uncertainty about the learned clusters.
% kmeans
This is a common trait among the clustering methods used in analyzing
...
be predetermined.
% hclust
Hierarchical clustering implementations typically suffer from
...
% model-based. flowClust
% nearest neighbors. phenograph
% 
