...
IBP (dIBP) to induce correlations between objects (rows) and to model multiple
...
IBP \citep{friggiths2011indian}.  Interactions between $J$ markers in
...
10454, 5177)$, for $J=32$ markers. Figure \ref{fig:cb-Y} shows the data. 
...
\beginmyfig
\includegraphics[scale=.2]{img/cb/Y001.png}
\includegraphics[scale=.2]{img/cb/Y002.png}
\includegraphics[scale=.2]{img/cb/Y003.png}
\caption{Cord blood marker expression data $y$ for sample 1 (left), sample 2
(middle), and sample 3 (right). Each column contains the expression levels of a
marker for all cells in the sample. High expression levels are red, low
expression levels are blue, missing values are white.}
\label{fig:cb-Y}
\endmyfig
Note that some columns (markers) consists of mostly missing values. For those
columns, the model will rely heavily on the prior missing mechanism
...
\ref{fig:cb-post-Z}.  In the top panel, the observations are sorted by a
posterior estimate of their cell-types, with the more predominant cell-types
placed toward the bottom.  The binary matrices in the lower panel show the 90\%
most frequent cell-types.
%
\beginmyfig
\includegraphics[scale=.2]{img/cb/YZ001.png}
\includegraphics[scale=.2]{img/cb/YZ002.png}
\includegraphics[scale=.2]{img/cb/YZ003.png}
\caption{CB data sorted by a point-estimate of their cell-types ($\lambda$), for
each sample.  Point-estimates of $\Z_i$ (sorted by $W_i$) and $W_i$ are provided
for each sample below each $y_i$.}
\label{fig:cb-post-Z}
\endmyfig
%
The model is able to group similar cells together and learn reasonable
underlying cell-types for those groups of cells. But it should be noted that
...
One way to assess model fit is through comparing the posterior predictive
distribution to the data. Since the data contains missing values, we compare
...
full dataset.  When applied to real cord blood data, we find that the
