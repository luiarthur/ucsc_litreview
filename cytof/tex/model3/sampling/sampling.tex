...
and $W$. The point estimates for $Z$ resemble $Z^\true$ and vary only rarely by
...
\caption{$y$ sorted by a point-estimate of their cell-types ($\lambda$), for each sample.
  Point-estimates of $\Z$ (sorted by $W_i$) and $W_i$ are provided for each sample below
  each $y_i$. 
}
...
\textbf{Posterior Predictive for $\y$}: TODO
