...
\(c_{ij}\) denote the ``cutoff'' values (provided by scientists) for
sample \(i\), marker \(j\). We consider the logarithm transformation
...
\(j\) of cell \(n\) in sample \(i\) \textbf{is missing}.
...
We introduce latent indicators for latent cell types, \(\lambda_{in}\),
\(i=1, \ldots, I\) and \(n=1, \ldots, N_i\).
\(\lambda_{in} \in \{1, \ldots, K\}\) denotes the cell phenotype of cell
\(n\) in sample \(i\) defined by column \(k\) of \(J \times K\) binary
matrix \(\Z\). Element \(z_{j, k} \in \{0, 1\}\) indicates if marker
\(j\) is expressed in cell phenotype \(k\). Event \(z_{j,k}=0\)
represents that marker \(j\) is not expressed, and \(z_{j,k}=1\) for
expression. Given \(z_{j, \lambda_{i,n}}\), we assume a mixture of
normals for \(y_{inj}\),
\begin{align*}
...
\(j=1, \ldots, J\),
\begin{eqnarray*}
p(\gamma_{inj} = \ell)=\eta^{z_{jk}}_{ij\ell}, \mbox{ where }~ \ell \in \{1,\ldots, L^{z_{jk}}\}.
\end{eqnarray*}
...
\beta_{1i} &\iid \N_+(m_{\beta_1}, s^2_{\beta_1}) ~~~~
\text{(hyper-parameters determined empirically)}, 
\end{align*}
...
\end{align*}
Note that \(X \sim \N_-(m,s^2)\) denotes that \(X\) is distributed Normally
with mean \(m\) and variance \(s^2\), truncated to take on only
\textbf{negative} values. Similarly, \(X \sim \N_+(m,s^2)\) denotes that
...
\subsection{\texorpdfstring{Full Conditional for
\(\bm\mu^*\)}{Full Conditional for \textbackslash{}bm\textbackslash{}mu\^{}*}}\label{full-conditional-for-bmmu}
...
\(S_{1\ell} = \bc{(i,n,j) : \p{Z_{j,\lin} = 1 ~\cap~ \gamma_{inj} = \ell}}\)
...
Then, for each posterior sample of \(Z\) and \(W_i\) (i.e. \(Z^{(b)}\)
