...
missing as follows; we simulated a proportion $p_{ij}$ of missing for marker
...
$$
P(\bZ \mid v, C) \propto \prod_{k=1}^K  \bc{\prod_{j=1}^J v_k^{z_{jk}}(1-v_k)^{1-z_{jk}}} \times
...
\sim \Gamma(a_\alpha, b_\alpha)$. Notice that $v_k$ has a conjugate prior.
The selection of $\phi$ requires some sensitivity analysis, but for now we
will fix $\phi=1$. Also note that the matrix-parameter $H$ in the previous
model is no longer in the model under this construction.
...
% TODO
We extend the model by letting phenotype abundances $\bw=(w_1, \ldots, w_K)$ be
a function of time $t$ after treatment and jointly analyze samples of a subject
...
dependent through time.  We also let $w_{t_i,k}$ exactly equal to zero for
some phenotypes. Let $\xi_{t_1,1}=1$ and assume $\xi_{t_1,k} =
