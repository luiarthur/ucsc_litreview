...
period of 1000 iterations. The results are summarized in Figure \ref{fig:cb-post-Z}.
In the top panel, the observations are sorted by a posterior estimate of their
cell-types, with the more predominant cell-types placed toward the bottom. 
The binary matrices in the lower panel show the 90\% most frequent cell-types.
%
\beginmyfig
...
\endmyfig
%
The model is able to group similar cells together and learn a reasonable
underlying cell-type for those groups of cells. But it should be noted that the
estimated $\bZ$ matrix for each sample contains different cell-types, but
within each sample, the cell-types may be repeated or vary by only one marker.
We would like the learned cell-types to be different rather than very similar
(or repeated). We can impose this constraint using a repulsive prior for the
cell-types ({\tt citation needed}).
...
\section{Posterior Predictive for CB data}
\foreach \ppp in {1,...,12}{
  \beginmyfig \includegraphics[page=\ppp]{img/cb/y_hist.pdf} \endmyfig
}

\section{Posterior Predictive for Positive values of CB data}
\foreach \ppp in {1,...,12}{
