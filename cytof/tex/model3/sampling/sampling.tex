...
Let \(\btheta=\{\bZ, \bw, \bm \mu^\star_0, \bm \mu^\star_1, \bm \sigma^2_{0i},
\bm \sigma^2_{1i}, \bm \eta^0, \bm \eta^1, \bm \lambda, \bm \gamma, \bm v, \bm
h, \bm \beta_0, \bm \beta_1\}\) represent all random parameters.  Let \(\y\)
and \(\m\) denote all \(y_{inj}\) and \(m_{inj}$ for all \((i,n,j)\),
respectively. The joint posterior distribution is 
\begin{align*}
...
conveniently, a metropolis step can be used.  Details of the posterior
...
phenotypes in rows. Black and white represents $z^\true_{jk}=0$ and 1,
...
\begin{center}
  \begin{tabular}{cc}
  \includegraphics[scale=.4]{img/FlowSOM/YZ001_FlowSOM.png}&
  \includegraphics[scale=.4]{img/FlowSOM/YZ002_FlowSOM.png}\\
  (a) Sample 1 & (b) Sample 2 \\
  \includegraphics[scale=.4]{img/FlowSOM/YZ003_FlowSOM.png} &\\
  (c) Sample 3 & \\
  \end{tabular}
\end{center}
\vspace{-0.05in}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.5]{img/FlowSOM/compareClus.pdf}
...
\begin{figure}
\begin{center}
  \includegraphics[scale=0.5]{img/cb/pz0_missy.pdf}
