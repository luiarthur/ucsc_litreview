...
%
\begin{itemize}
  \item $F_{0ij} = \sum_{\ell=1}^{L_0} \eta^0_{ij\ell}~\text{Normal}(\mu^\star_{0\ell}, \sigma^2_i)$, where $\mu^\star_{0\ell} < 0$
  \item $F_{1ij} = \sum_{\ell=1}^{L_1} \eta^1_{ij\ell}~\text{Normal}(\mu^\star_{1\ell}, \sigma^2_i)$, where $\mu^\star_{1\ell} > 0$
\end{itemize}
%
\begin{figure}
  \begin{center}
...
\tightlist
\item
  Fixed number of mixture components \(L_0\) and \(L_1\)
\item
  Mixture weights \(\bet^0_{ij}\) and \(\bet^1_{ij}\) where
  \(\sum_{\ell=1}^{L_0} \eta^0_{ij\ell}=\sum_{\ell=1}^{L_1}\eta^1_{ij\ell}=1\),
...
\item
  ...
  \[ P(\gamma_{inj} = \ell \mid \lin=k)=\eta^{z_{jk}}_{ij\ell}, \mbox{ where }~ \ell \in \{1,\ldots, L_{z_{jk}}\}.\]
  Then, \[ 
